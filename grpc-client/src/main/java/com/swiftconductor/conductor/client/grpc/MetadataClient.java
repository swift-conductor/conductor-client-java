...
public class MetadataClient extends ClientBase {
    ...
    public void registerWorkflowDef(WorkflowDef workflowDef) {
        ...
        stub.createWorkflow(MetadataServicePb.CreateWorkflowRequest.newBuilder()
                .setWorkflow(protoMapper.toProto(workflowDef)).build());
    }
     ...
     *
     * @param workflowDefs
     *            List of workflow definitions to be updated
     */
    public void updateWorkflowDefs(List<WorkflowDef> workflowDefs) {
        ...
        stub.updateWorkflows(MetadataServicePb.UpdateWorkflowsRequest.newBuilder()
                .addAllDefs(workflowDefs.stream().map(protoMapper::toProto)::iterator).build());
    }
     ...
     * @param version
     *            the version of the workflow def
    ...
    public WorkflowDef getWorkflowDef(String name, @Nullable Integer version) {
        ...
        MetadataServicePb.GetWorkflowRequest.Builder request = MetadataServicePb.GetWorkflowRequest.newBuilder()
                .setName(name);
     ...
     *
     * @param taskDefs
     *            List of task types to be registered.
     */
    public void registerTaskDefs(List<TaskDef> taskDefs) {
        ...
        stub.createTasks(MetadataServicePb.CreateTasksRequest.newBuilder()
                .addAllDefs(taskDefs.stream().map(protoMapper::toProto)::iterator).build());
    }
     ...
     *
     * @param taskDef
     *            the task definition to be updated
     */
    public void updateTaskDef(TaskDef taskDef) {
        Preconditions.checkNotNull(taskDef, "Task definition cannot be null");
        stub.updateTask(MetadataServicePb.UpdateTaskRequest.newBuilder().setTask(protoMapper.toProto(taskDef)).build());
    }
     ...
     *
     * @param taskType
     *            type of task for which to retrieve the definition
    ...
    public TaskDef getTaskDef(String taskType) {
        ...
        return protoMapper.fromProto(
                stub.getTask(MetadataServicePb.GetTaskRequest.newBuilder().setTaskType(taskType).build()).getTask());
    }
     ...
     *
     * @param taskType
     *            Task type to be unregistered.
     */
    public void unregisterTaskDef(String taskType) {
        Preconditions.checkArgument(StringUtils.isNotBlank(taskType), "Task type cannot be blank");
        stub.deleteTask(MetadataServicePb.DeleteTaskRequest.newBuilder().setTaskType(taskType).build());
    }
}

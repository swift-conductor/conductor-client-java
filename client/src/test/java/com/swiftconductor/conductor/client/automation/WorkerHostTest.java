...
public class WorkerHostTest {
    ...
    public void testNoWorkersException() {
        new WorkerHost.Builder(null, null).build();
    }

    @Test(expected = ClientException.class)
    public void testInvalidThreadConfig() {
        AbstractWorker worker1 = AbstractWorker.create("task1", TaskResult::new);
        AbstractWorker worker2 = AbstractWorker.create("task2", TaskResult::new);
        Map<String, Integer> taskThreadCount = new HashMap<>();
        taskThreadCount.put(worker1.getTaskDefName(), 2);
        taskThreadCount.put(worker2.getTaskDefName(), 3);
        new WorkerHost.Builder(client, Arrays.asList(worker1, worker2)).withThreadCount(10)
    ...
    public void testMissingTaskThreadConfig() {
        ...
        Map<String, Integer> taskThreadCount = new HashMap<>();
        taskThreadCount.put(worker1.getTaskDefName(), 2);
        WorkerHost configurer = new WorkerHost.Builder(client, Arrays.asList(worker1, worker2))
                .withTaskThreadCount(taskThreadCount).build();
    ...
    public void testSharedThreadPool() {
        AbstractWorker worker = AbstractWorker.create(TEST_TASK_DEF_NAME, TaskResult::new);
        WorkerHost configurer = new WorkerHost.Builder(client, Arrays.asList(worker, worker, worker)).build();
        configurer.init();
        ...
        configurer = new WorkerHost.Builder(client, Collections.singletonList(worker)).withThreadCount(100)
                .withSleepWhenRetry(100).withUpdateRetryCount(10).withShutdownGracePeriodSeconds(15)
                .withWorkerNamePrefix("test-worker-").build();
    ...
    public void testMultipleWorkersExecution() throws Exception {
        ...
        AbstractWorker worker1 = mock(AbstractWorker.class);
        ...
        AbstractWorker worker2 = mock(AbstractWorker.class);
        ...
        WorkerHost configurer = new WorkerHost.Builder(taskClient, Arrays.asList(worker1, worker2)).withThreadCount(2)
                .withSleepWhenRetry(100000).withUpdateRetryCount(1).withWorkerNamePrefix("test-worker-").build();
        when(taskClient.batchPollTasksInDomain(any(), any(), any(), anyInt(), anyInt())).thenAnswer(invocation -> {

...
public class WorkerProcessTest {
    ...
    private static final Map<String, Integer> TASK_THREAD_MAP = Collections.singletonMap(TEST_TASK_DEF_NAME, 1);

    @Test
    public void testTaskExecutionException() throws InterruptedException {
        AbstractWorker worker = AbstractWorker.create(TEST_TASK_DEF_NAME, task -> {
            throw new NoSuchMethodError();
        });
        TaskClient taskClient = Mockito.mock(TaskClient.class);
        WorkerProcess taskPollExecutor = new WorkerProcess(null, taskClient, 1, new HashMap<>(), "test-worker-%d",
                TASK_THREAD_MAP);
        ...
        Executors.newSingleThreadScheduledExecutor().scheduleAtFixedRate(() -> taskPollExecutor.pollAndExecute(worker),
    ...
    public void testMultipleTasksExecution() throws InterruptedException {
        ...
        when(worker.execute(any())).thenAnswer(new Answer() {
            ...
            public TaskResult answer(InvocationOnMock invocation) throws InterruptedException {
        ...
        TaskClient taskClient = Mockito.mock(TaskClient.class);
        WorkerProcess taskPollExecutor = new WorkerProcess(null, taskClient, 1, new HashMap<>(), "test-worker-",
                TASK_THREAD_MAP);
        when(taskClient.batchPollTasksInDomain(any(), any(), any(), anyInt(), anyInt()))
    ...
    public void testLargePayloadCanFailUpdateWithRetry() throws InterruptedException {
        ...
        // When evaluateAndUploadLargePayload fails indefinitely, task update shouldn't
        // be called.
    ...
    public void testLargePayloadLocationUpdate() throws InterruptedException {
        ...
        doAnswer(invocation -> {
            ...
            assertEquals(largePayloadLocation, result.getExternalOutputPayloadStoragePath());
            latch.countDown();
            return null;
        }).when(taskClient).updateTask(any());
    ...
    public void testTaskPollException() throws InterruptedException {
        ...
        TaskClient taskClient = Mockito.mock(TaskClient.class);
        when(taskClient.batchPollTasksInDomain(any(), any(), any(), anyInt(), anyInt()))
                .thenThrow(ClientException.class).thenReturn(Arrays.asList(task));
    ...
    public void testTaskPollDomain() throws InterruptedException {
        ...
        WorkerProcess taskPollExecutor = new WorkerProcess(null, taskClient, 1, taskToDomain, "test-worker-",
                TASK_THREAD_MAP);
    ...
    public void testPollOutOfDiscoveryForTask() throws InterruptedException {
        ...
        TaskClient taskClient = Mockito.mock(TaskClient.class);
        when(taskClient.batchPollTasksInDomain(any(), any(), any(), anyInt(), anyInt()))
                .thenReturn(Arrays.asList(new Task())).thenReturn(Arrays.asList(task));

        WorkerProcess taskPollExecutor = new WorkerProcess(client, taskClient, 1, new HashMap<>(), "test-worker-",
    ...
    public void testBatchTasksExecution() throws InterruptedException {
        ...
        for (int i = 0; i < threadCount; i++) {
            ...
            when(worker.execute(task)).thenAnswer(new Answer() {
                Map<String, Object> outputMap = new HashMap<>();
        ...
        when(taskClient.batchPollTasksInDomain(TEST_TASK_DEF_NAME, null, workerName, threadCount, 1000))
                .thenReturn(tasks);
        when(taskClient.ack(any(), any())).thenReturn(true);

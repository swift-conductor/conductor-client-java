...
public class AnnotatedWorker implements AbstractWorker {
    ...
    private Set<TaskResult.Status> failedStatuses = Set.of(TaskResult.Status.FAILED,
    ...
    private Object getInputValue(Task task, Class<?> parameterType, Type type, Annotation[] paramAnnotation) {
    ...
    private static InputParam findInputParamAnnotation(Annotation[] paramAnnotation) {
        return (InputParam) Arrays.stream(paramAnnotation).filter(ann -> ann.annotationType().equals(InputParam.class))
    ...
    private TaskResult setValue(Object invocationResult, TaskResult result) {
        ...
        if (opAnnotation != null) {
            ...
            List<com.swiftconductor.conductor.sdk.workflow.def.tasks.Task<?>> tasks = forkInput.getTasks();
            List<WorkflowTask> workflowTasks = new ArrayList<>();
            for (com.swiftconductor.conductor.sdk.workflow.def.tasks.Task<?> sdkTask : tasks) {
    ...
    public void setPollingInterval(int pollingInterval) {
        System.out.println("Setting the polling interval for " + getTaskDefName() + ", to " + pollingInterval);

...
public class WorkflowTestFrameworkTests {
    ...
    public void testDynamicTaskExecuted() throws Exception {
        ...
        CompletableFuture<Workflow> completableFuture = manager.startWorkflow("Decision_TaskExample", 1, input);
        ...
        // task_2's implementation fails at the first try, so we should have to
        // instances of task_2
        // execution
        ...
        List<Task> task2Executions = workflow.getTasks().stream().filter(task -> task.getTaskDefName().equals("task_2"))
